...
struct cfdriver uwacom_cd = { NULL, "uwacom", DV_DULL };
...
const struct cfattach uwacom_ca = { sizeof(struct uwacom_softc), uwacom_match,
	uwacom_attach, uwacom_detach };
...
uwacom_match(struct device *parent, void *match, void *aux) {
	...
	if (usb_lookup(uwacom_devs, uha->uaa->vendor, uha->uaa->product)
		== NULL)
		return (UMATCH_NONE);
	...
	if (hid_is_collection(desc, size, uha->reportid, HID_USAGE2(HUP_WACOM | HUP_DIGITIZERS, HUG_POINTER)))
		return UMATCH_IFACECLASS;

	if (!hid_locate(desc, size, HID_USAGE2(HUP_WACOM | HUP_DIGITIZERS, HUG_POINTER),
...
static void print_hid_loc(struct hid_location hl)
{
	printf("size: 0x%x count: 0x%x pos: 0x%x\n", hl.size, hl.count, hl.pos);
}

void print_tsscale(struct tsscale scale) {
    printf("minx: %d\n", scale.minx);
    printf("maxx: %d\n", scale.maxx);
    printf("miny: %d\n", scale.miny);
    printf("maxy: %d\n", scale.maxy);
...
void print_hidms(struct hidms ms) {
    printf("sc_enabled: %d\n", ms.sc_enabled);
    printf("sc_flags: 0x%x\n", ms.sc_flags);
    printf("sc_num_buttons: %d\n", ms.sc_num_buttons);
    printf("sc_buttons: %u\n", ms.sc_buttons);
    printf("sc_loc_x: ");
    print_hid_loc(ms.sc_loc_x);
    printf("sc_loc_y: ");
    print_hid_loc(ms.sc_loc_y);
    printf("sc_loc_z: ");
    print_hid_loc(ms.sc_loc_z);
    printf("sc_loc_w: ");
    print_hid_loc(ms.sc_loc_w);
    for (int i = 0; i < ms.sc_num_buttons; i++) {
        printf("sc_loc_btn[%d]: ", i);
        print_hid_loc(ms.sc_loc_btn[i]);
    }
    printf("sc_tsscale: ");
    print_tsscale(ms.sc_tsscale);
    printf("sc_rawmode: %d\n", ms.sc_rawmode);
}
